...
class resizeNormalizeKeepRatio(object):
    ...
    def __call__(self, img, label_str):
        ...
        return img_tensor, torch.tensor(
            cv2.resize(re_mask_cpy, (self.size[0] * 2, self.size[1] * 2), cv2.INTER_NEAREST)).float()
...
class alignCollate_syn_withcrop(object):
    ...
    def __call__(self, batch):
        ...
        images_lr = [image.resize((image.size[0] // self.down_sample_scale, image.size[1] // self.down_sample_scale),
                                  Image.BICUBIC) for image in images]
        images_lr = [self.transform2(image) for image in images_lr]
        images_lr = torch.cat([t.unsqueeze(0) for t in images_lr], 0)
...
class alignCollate_realWTLAMask(alignCollate_syn):
    ...
    def __call__(self, batch):
        ...
        return images_HR, images_pseudoLR, images_lr, images_HRy, images_lry, label_strs, label_rebatches, torch.tensor(
            weighted_masks).long(), torch.tensor(weighted_tics)
...
if __name__ == '__main__':
    ...
    root_path = "/home/hjy/DATA/TATT/TextZoom/train2/"

    data_annos = lmdbDataset_real(root_path)
    nsamples = data_annos.nSamples

    save_dir = "/home/hjy/DATA/TATT/TextZoom/canny/"
    ...
    for i in range(300):
        ...
        pub_img[lr_canny.shape[0] * 2 + 10:lr_canny.shape[0] * 2 + 10 + hr_canny.shape[0],
        :hr_canny.shape[1]] = img_hr_gray
